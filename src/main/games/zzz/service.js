...
const saveData = async (data) => {
  ...
  await config.save()
  await saveJSON(`zzz-gacha-list-${data.uid}.json`, obj)
}
...
const findDataFiles = async (dataPath, fileMap) => {
  ...
  if (files?.length) {
    for (let name of files) {
      if (/^zzz-gacha-list-\d+\.json$/.test(name) && !fileMap.has(name)) {
...
const readLog = async () => {
  ...
  try {
    ...
    const promises = logPaths.map(async logpath => {
      ...
      const directUrlMch = logText.match(/https:\/\/.*?\/info/g)
      if (directUrlMch) {
        return getLatestUrl(directUrlMch)
      }
...
const getGachaLogs = async ({ name, key }, queryString) => {
  ...
  do {
    ...
    if (!region_time_zone) {
      region_time_zone = res.region_time_zone || serverTimeZone.get(res.region) || 8
    }
    list.push(...logs)
    page += 1
...
const tryGetUid = async (queryString) => {
  ...
  try {
    for (let [key] of defaultTypeMap) {
      const res = await request(`${url}&real_gacha_type=${key}&page=1&size=6&end_id=0`)
...
const fetchData = async (urlOverride) => {
  ...
  const gachaType = await getGachaType(searchParams.get('lang')) || [...defaultTypeMap].map(([key, name]) => ({ key, name }))
...
const getData = () => ({ dataMap, current: config.current })
