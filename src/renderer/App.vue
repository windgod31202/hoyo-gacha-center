  ...
  <div v-if="ui" class="app-shell min-h-screen text-slate-100">
    <aside class="sidebar">
      ...
      <div class="game-switcher">
        <button
          ...
          :class="[
            'game-button',
            {
              active: state.game === game.key,
              pending: state.pendingGame === game.key
            }
          ]"
      ...
      <div class="side-panel">
        ...
        <div v-else class="current-uid">
          {{ state.current ? maskUid(state.current) : '尚未載入' }}
        </div>
      </div>
    ...
    <main class="main-content">
      ...
      <header class="hero-card">
        ...
        <div class="header-actions">
          <el-button plain @click="newUser" :disabled="state.status === 'loading'">
            新增帳號
          </el-button>
      ...
      >
        <article
          ...
          :key="item[0]"
          :class="{ hidden: state.config.hideNovice && item[0] === currentGameMeta.noviceType }"
        >
          <div class="pool-header">
            <div>
              ...
              <span>
                {{ item[1].total }} 抽 ·
                {{ rarityLabels.top }} {{ item[1].count5 }} ·
                目前保底 {{ item[1].countMio }}
              </span>
            </div>
          </div>
    ...
    <el-dialog
      :title="ui.urlDialog?.title || '手動貼上 URL'"
      v-model="state.showUrlDlg"
      width="90%"
      class="max-w-md"
    >
      <p class="mb-4 text-gray-500">
        請貼上目前遊戲的祈願 / 躍遷 / 調頻紀錄 URL，程式會依照左側選擇的遊戲匯入。
      </p>
    ...
    >
      ...
      <p class="my-2 flex flex-col text-teal-800 text-[13px]">
        <span class="my-1" v-for="txt of cacheCleanTextList" :key="txt">
          {{ txt }}
        </span>
      </p>
      ...
      <template #footer>
        <div class="dialog-footer text-center">
          <el-button type="primary" @click="state.showCacheCleanDlg = false">
            確定
          </el-button>
        </div>
      </template>
...
const currentGameMeta = computed(() => {
  return games.find(item => item.key === state.game) || games[1]
})

const pendingGameMeta = computed(() => {
  return games.find(item => item.key === state.pendingGame) || currentGameMeta.value
})

const pageTitle = computed(() => currentGameMeta.value.title)

const pageLoading = computed(() => {
  return state.status === 'loading' || detailLoading.value
})

const loadingTitle = computed(() => {
  if (state.pendingGame) {
    return `正在切換到 ${pendingGameMeta.value.name}`
  }
...
const loadingText = computed(() => {
  ...
  if (detailLoading.value) return '正在重新計算卡池、稀有度與保底資料...'
  return '請稍候，正在讀取抽卡紀錄...'
})

const rarityLabels = computed(() => {
  return state.game === 'zzz'
    ? {
        top: 'S 級',
        mid: 'A 級',
        avg: '平均 S 級抽數',
...
const gachaData = computed(() => {
  return dataMap.value.get(state.current)
})

const uidSelectText = computed(() => {
  return state.current === 0 ? '新增帳號' : state.current
})
...
const hint = computed(() => {
  ...
  if (state.status === 'init') {
    return `選擇 ${currentGameMeta.value.name} 後載入資料。`
  }

  if (state.status === 'loaded' && data) {
    return `${text.lastUpdate}${colon}${new Date(data.time).toLocaleString()}`
  }
...
const typeMap = computed(() => {
  ...
  if (state.game === 'genshin') {
    list =
      genshinGachaType[lang] ||
      genshinGachaType['zh-tw'] ||
      genshinGachaType['zh-cn'] ||
      []
  } else if (state.game === 'zzz') {
    const gachaTypeMap = new Map(zzzGachaType)

    list =
      gachaTypeMap.get(lang) ||
      gachaTypeMap.get('zh-tw') ||
      gachaTypeMap.get('zh-cn') ||
      []
  } else {
...
const summary = computed(() => {
  ...
  result.avg5 = fivePities.length
    ? Math.round((fivePities.reduce((a, b) => a + b, 0) / fivePities.length) * 10) / 10
    : 0

  result.fourRate = result.total
    ? Math.round((result.four / result.total) * 1000) / 10
    : 0
...
const saveExcel = async () => {
  return ipcRenderer.invoke('SAVE_EXCEL', state.game)
}

const openCacheFolder = async () => {
  return ipcRenderer.invoke('OPEN_CACHE_FOLDER', state.game)
}
...
const setTitle = () => {
  document.title = `HoYo Gacha Center - v${version}`
}
...
const copyUrl = async () => {
  ...
  if (successed) {
    ...
    ElMessage.error(state.i18n?.log?.url?.notFound || '找不到 URL')
  }
}
